...
// Show on Menu
if (isset($_GET["menu"])) { // If the user change the status of show on menu
   // Get the record id
   if (!$recordID = securityCheck($_GET['id'], 'number')) {
      die('Error in the record id');
   }
   // Get the menu value (Yes/No)
   if (!$showmenu = securityCheck($_GET['menu'])) {
      die('Error in the show value');
   }

   // Update the record
   $updateshowonmenu = $VIEW->updateShowOnMenu($recordID, $table['name'], $showmenu);

   $logtable = $table['name'];
   $action   = "Show/Hide Record On Menu ".$recordID." in Form " . $formnumber ;

   // Build the SQL string for logging (don't try to read it from the bool)
   $rawQuery = "UPDATE `{$logtable}` SET `showonmenu` = '{$showmenu}' WHERE `id` = {$recordID}";
   $sqlquery = mysqli_real_escape_string($conn, $rawQuery);

   $notes    = "Toggling Show/Hide on Menu ".$recordID."";
   $username = $_SESSION["useremail"];
...
<body>
   ...
   <section id="main-content">
      <section class="wrapper site-min-height">
                        ...
                        <tbody id="sortable">
                           ...
                           if ($form) {
                              ...
                              foreach ($tableView as $row) {
                                 ...
                                 $showonmenu = isset($row["showonmenu"]) ? $row["showonmenu"] : null;
         ...
         } elseif ($act['id'] == 6) {               // Show/Hide on Menu  <-- use your ID here
            $ex_icons = explode(",", $act['icon']);
            if ($showonmenu == "Yes") {
               $menu = "No";
               $icon = $VIEW->getIconById($ex_icons[0]);
         ...
         // --- LINK RESOLUTION ---
                                          ...
                                          if ($act['link'] == 'Yes') {
                                             ...
                                             // Decide which toggle value to pass to replaceURL
            $toggleValue = null;
            if ($act['id'] == 4) {
               $toggleValue = $show;
            } elseif ($act['id'] == 6) {
               $toggleValue = $menu;
                                             ...
                                             $link = replaceURL($act['link_href'], $formnumber, $row["id"], $toggleValue);
...
<script>
   ...
   // ✅ Rebuild second header row properly with <th> cells
   var $thead = $('#blogTable thead');
   if ($thead.find('tr').length === 1) {
      var $first = $thead.find('tr:first');
      var $clone = $('<tr></tr>');
      $first.find('th').each(function() {
         $clone.append('<th></th>');
      });
      $thead.append($clone);
   } else {
      // convert existing <td> row to <th> for consistency
      var $second = $thead.find('tr:eq(1)');
      if ($second.find('td').length > 0) {
         var html = '';
         $second.find('td').each(function() { html += '<th></th>'; });
         $second.replaceWith('<tr>' + html + '</tr>');
      }
   }

   var table = $('#blogTable').DataTable({
      stateSave: true,
      // DataTables' default key uses only the script path, so its filters
      // were shared by recordViewv4.php?frm=2, ?frm=18, and every other CMS
      // form. Keep saved filters, ordering and pagination per CMS form.
      stateSaveCallback: function(settings, data) {
         localStorage.setItem('DataTables_recordView_form_<?php echo (int) $formnumber; ?>', JSON.stringify(data));
      },
      stateLoadCallback: function(settings) {
         var savedState = localStorage.getItem('DataTables_recordView_form_<?php echo (int) $formnumber; ?>');
      ...
      pageLength: <?php echo ($user["recordstoshow"] > 0 ? $user['recordstoshow'] : 15); ?>,

      initComplete: function() {
         const api = this.api();

         // --- SELECT filters ---
         api.columns([<?php echo $selectCount; ?>]).every(function() {
         const column = this;
         const $th = $('#blogTable thead tr:eq(1) th').eq(column.index());
         ...
         // --- TEXT SEARCH filters ---
         api.columns([<?php echo $searchCount; ?>]).every(function() {
         const column = this;
         const $th = $('#blogTable thead tr:eq(1) th').eq(column.index());
   ...
   if ($('#blogTable_wrapper .dt-buttons').length) {
      table.buttons().container().appendTo('#blogTable_wrapper .dt-buttons');
   } else if ($('#blogTable_wrapper .col-md-3:eq(0)').length) {
      table.buttons().container().appendTo('#blogTable_wrapper .col-md-3:eq(0)');
   } else {
      table.buttons().container().insertAfter('#blogTable');
   }
