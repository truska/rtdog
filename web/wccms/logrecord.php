...
{
    global $conn; // ✅ Make the $conn variable available

    // error_log("Start of save function CONTENTID: ".$contentid) ;
    ...
    $sqlquerydata = mysqli_real_escape_string($conn, (string) $sqlquery);
    $form = mysqli_real_escape_string($conn, (string) $form);
    $result = mysqli_real_escape_string($conn, (string) $result);
    $notes = mysqli_real_escape_string($conn, (string) $notes);
    $ip = mysqli_real_escape_string($conn, (string) $ip);
    $browser = mysqli_real_escape_string($conn, (string) $browser);
    $userAgent = mysqli_real_escape_string($conn, (string) $userAgent);
    $contentid = (int) $contentid;

    $sqlinsertlog = "INSERT INTO `cms_log` (`username`, `datetime`, `result`, `contentid`, `action`, `query`, `notes`, `ip`, `browser`, `agent`, `form`) VALUES ('$username', NOW(), '$result', '$contentid', '$action', '$sqlquerydata', '$notes', '$ip', '$browser', '$userAgent', '$form')";

    $queryinsertlog = mysqli_query($conn, $sqlinsertlog);

    if ($queryinsertlog) {
        // echo "<script>alert('Log Updated ! $sqlinsertlog ')</script>";
        // error_log("Log Updated OK: ".$sqlquerydata);
    } else {
        // echo "<script>alert('Error writing to log ! $sqlinsertlog ')</script>";
        // error_log("Error writing to log: ".$sqlquerydata);
    }
