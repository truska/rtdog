<?php
   // This endpoint is consumed by Dropzone, so PHP notices must not corrupt
   // its JSON response.
   ...
   // main-top-files honours the CMS debug preference and can turn display_errors
   // back on. This endpoint must always return clean JSON to Dropzone.
   ini_set('display_errors', 0);
   ini_set('display_startup_errors', 0);
   error_reporting(E_ALL);
   /*
...
*/
   ...
   if (isset($_FILES['images'])) {
      ...
      $images_size_filter = 8000000; // 8MB
      $errors = array();
      ...
      // Dropzone sends one image per request, while the fallback control may
      // send many. Normalise both shapes to the same array structure.
      if (!is_array($_FILES['images']['tmp_name'])) {
         foreach (['name', 'size', 'tmp_name', 'type', 'error'] as $property) {
            $_FILES['images'][$property] = [$_FILES['images'][$property]];
         }
      }

      // Loop through each image
      foreach ($_FILES['images']['tmp_name'] as $key => $tmp_name) {
         if (($_FILES['images']['error'][$key] ?? UPLOAD_ERR_OK) !== UPLOAD_ERR_OK) {
            $errors[] = 'The file ' . ($_FILES['images']['name'][$key] ?? 'image') . ' could not be uploaded (PHP upload error ' . $_FILES['images']['error'][$key] . ').';
            continue;
         }
      ...
      $errors[] = "No images were uploaded. The selected batch exceeds the 20 MB upload limit; please select fewer images or reduce their file sizes, then try again.";
   }
