...
class ImageResizer
{
   ...
   public function saveInDatabase($image_name, $id, $form, $formName)
   {
      ...
      // The gallery table is strict-mode compatible: these metadata fields are
      // required even when the editor has not collected values for them yet.
      $query = "INSERT INTO `gallery`
      (`record_id`, `form_id`, `form_name`, `name`, `alttag`, `caption`, `date`, `image`, `folder_name`, `slug`, `keyword`, `sort`)
      VALUES ('$id', '$form', '$formName', '$name', '', '', NOW(), '$image_name', '$destination_folder', '', '', '$sort')";
