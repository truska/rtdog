...
class RecordEdit
{
   ...
   function __construct($form_id, $record_id,$dataform)
   {
      ...
      if ($form) {
         ...
         $this->showgallery = $form['showgallery'] ?? null;
...
public function getCss()
{
   ...
   {
      $query = "SELECT * FROM `gallery`
      WHERE form_id = 2 AND record_id = " . $this->record_id . " 
      ORDER BY sort ASC";
   ...
   {
      $alttag = mysqli_real_escape_string(DB::connection(), $data['alttag'] ?? '');
      $title = mysqli_real_escape_string(DB::connection(), $data['title'] ?? '');
      $caption = mysqli_real_escape_string(DB::connection(), $data['caption'] ?? '');
      $includeInGallery = ($data['includeingallery'] ?? 'No') === 'Yes' ? 'Yes' : 'No';
      $gallerySort = max(0, (int) ($data['gallerysort'] ?? 0));
        ...
        foreach ($data as $key => $value) {
         if ($key != 'formnumber' && $key != 'submit') {
            ...
            if ($fieldType == 2) {
             ...
             // A select option may explicitly represent an unknown value.
             // Store it as database NULL rather than the sentinel text.
             if ($fieldType === 16 && $value === '__NULL__') {
                 $query .= "`$key` = NULL, ";
                 continue;
             }

             // Optional number fields (such as health scores) are submitted as
             // an empty string when unset.  In strict MySQL mode that cannot be
             // written to an integer column, so retain the intended unknown value.
             ...
             $safeValue = mysqli_real_escape_string(DB::connection(), (string) $value);
             $query .= "`$key` = '$safeValue', ";
         }
     }
