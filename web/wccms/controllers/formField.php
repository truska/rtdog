...
class FormField
{
   ...
   */
   ...
   {
       // Skip escaping for TinyMCE / WYSIWYG field type (ID 19)
       if (isset($this->rowfield['id']) && (int)$this->rowfield['id'] === 19) {
           return $text;
       }
   
       // You could also skip for plain textareas if needed (ID 20)
       // if (isset($this->rowfield['id']) && in_array((int)$this->rowfield['id'], [19, 20])) {
       //     return $text;
       // }
   ...
   protected function renderSelectField()
   {
      if ($this->rowformfield["allowedit"] == 'Yes') {
         ...
         // Do not let an empty select silently submit its first option (for
         // example, assigning a DM status of Clear to an unrecorded dog).
         if (($this->ContentValue === null || $this->ContentValue === '') && !$hasNullOption) {
            $output .= "<option value='' selected disabled>Select an option</option>";
         }

         foreach ($fieldOptions as $fOption) {
            $isNullSelection = ($fOption['value'] ?? '') === '__NULL__'
               && ($this->ContentValue === null || $this->ContentValue === '');
            if ($isNullSelection || $this->ContentValue == $fOption["value"]) {
   ...
   {
      $dropzoneId = 'gallery-dropzone-' . (int) $formnumber . '-' . (int) $recordnumber;
      $submitId = 'gallery-submit-' . (int) $formnumber . '-' . (int) $recordnumber;
      $responseId = 'gallery-response-' . (int) $formnumber . '-' . (int) $recordnumber;

      $output = '<form action="controllers/image_processor.php?id=' . $recordnumber . '&frm=' . $formnumber . '" method="post" enctype="multipart/form-data" class="dropzone" id="' . $dropzoneId . '">
         <div class="fallback">
            <input type="file" name="images[]" multiple>
			</div>
		</form>';
      $output .= "<button type='button' class='btn btn-success' id='$submitId'>Upload Images</button>";
      $output .= "<div id='$responseId' style='background-color: white;'>
      </div>";
      $output .= '<script>
      (function () {
         ...
         if (!dropzoneElement || !submitButton || typeof Dropzone === "undefined") {
            return;
         }

         // Keep the selected gallery together in one request so one click
         // uploads every image. The server batch allowance is configured in
         // web/.user.ini.
         Dropzone.autoDiscover = false;
         var galleryDropzone = new Dropzone(dropzoneElement, {
         paramName: "images",
         acceptedFiles: ".jpg,.jpeg,.png,.gif", // Extensiones de archivo aceptadas
         maxFilesize: 8,
         dictFileTooBig: "This image is too large ({{filesize}} MB). The maximum is 8 MB per image.",
         ...
         maxFiles: '. (int) $this->maxGalleryImages .', // Número máximo de archivos
         parallelUploads: '. (int) $this->maxGalleryImages .',
         uploadMultiple: true
         });

         var uploadFailed = false;
         submitButton.addEventListener("click", function() {
            if (galleryDropzone.getQueuedFiles().length === 0) {
               responseElement.textContent = "Please select one or more images first.";
               return;
            }
            uploadFailed = false;
            responseElement.textContent = "Uploading images…";
         ...
         galleryDropzone.on("error", function (file, message, xhr) {
            ...
            if (typeof message === "string") {
               ...
               responseElement.textContent = message.errors.join(" ");
            } else if (message && message.message) {
               responseElement.textContent = message.message;
            } else {
               responseElement.textContent = "The image could not be uploaded. Please try again.";
            }
         });
         galleryDropzone.on("queuecomplete", function () {
            if (!uploadFailed) {
               responseElement.textContent = "Images uploaded successfully. Reloading gallery…";
      ...
      // No gallery images is a normal state for a new record.
      ...
      if ($formnumber == 11) { // Product
         ...
         foreach ($images_data as $resimglist) {
            ...
            $title = $resimglist["title"] ?? '';
            $img_caption = $resimglist["caption"];
            $includeingallery = ($resimglist['includeingallery'] ?? 'No') === 'Yes' ? 'checked' : '';
            $gallerysort = (int) ($resimglist['gallerysort'] ?? 0);
            ...
            $output .= "<tr id='$img_id'>
               ...
               <td>
                  ...
                  Title <input type='text' class='form-control w-25' value='$title' name='title-$img_id'>
                  Caption <input type='text' class='form-control w-25 ' value='$img_caption' name='caption-$img_id'>
                  <label class='mt-2 d-block'>Gallery order <input type='number' min='0' class='form-control d-inline-block w-auto' value='$gallerysort' name='gallerysort-$img_id'></label>
                  <div class='form-check mt-2'>
                     <input type='checkbox' $includeingallery class='form-check-input' id='includeingallery-$img_id' name='includeingallery-$img_id'>
                     <label class='form-check-label' for='includeingallery-$img_id'>Include in site gallery</label>
                  </div>
