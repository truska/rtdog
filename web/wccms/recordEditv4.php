...
<?php
	...
	if (isset($_POST['submit'])) {
		...
		$redirectTo = trim((string) $form['afterEdit']);
	...
	if (isset($_POST["updateGallery"])) {
		...
		foreach ($images as $proimg) {
			...
			$data['title'] = $_POST["title-{$data['img_id']}"] ?? '';
			$data['caption'] = $_POST["caption-{$data['img_id']}"] ?? '';
			$data['includeingallery'] = isset($_POST["includeingallery-{$data['img_id']}"]) ? 'Yes' : 'No';
			$data['gallerysort'] = $_POST["gallerysort-{$data['img_id']}"] ?? 0;
...
<body>
		...
		<section id="main-content">
			<section class="wrapper site-min-height">
				...
				<div class="row">
					<div class="col-lg-12">
						<?php
						// Product records use their name. Other record types retain a
						// sensible fallback without relying on table-column order.
						$recordLabel = $rowcontent['name'] ?? $rowcontent['heading'] ?? $rowcontent['title'] ?? '';
						if ($recordLabel === '') {
							$recordLabel = (string) reset($rowcontent);
						}
						$recordLabel = htmlspecialchars(stripslashes((string) $recordLabel), ENT_QUOTES, 'UTF-8');
						$formTitle = htmlspecialchars((string) $form['title'], ENT_QUOTES, 'UTF-8');
						?>

						<h2><span style="font-weight:200;">Edit <?= $formTitle ?></span>:
							<?= $recordLabel ?> <sup>(id: <?= (int) $recordnumber ?>)</sup></h2>
							...
							<div class="card-body">
								...
								<ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
									...
									//	echo "hello" ;
										...
										$tabSlug = strtolower($tab['name']);
										$tabSlug = preg_replace('/[^a-z0-9]+/', '-', $tabSlug);
										$tabSlug = trim($tabSlug, '-');

										echo "<li class='nav-item mr-1'>";

										echo "<a class='nav-link " . ($tab['tabID'] == 1 ? "active" : "") . "' id='pills-{$tabSlug}-tab' data-bs-toggle='pill' href='#pills-{$tabSlug}' role='tab' aria-controls='pills-{$tabSlug}' aria-selected='false'><i class='$iconTab'></i> {$tab['name']}</a>";
								...
								<div class="tab-content" id="pills-tabContent">
									...
									foreach ($tabs as $tab) {
										...
										$tabSlug = preg_replace('/[^a-z0-9]+/', '-', $tabSlug);
										$tabSlug = trim($tabSlug, '-');
										echo "<div class='tab-pane fade " . ($tab['tabID'] == 1 ? "active show" : '') . "' id='pills-{$tabSlug}' role='tabpanel' aria-labelledby='pills-{$tabSlug}-tab'>";
				...
				document.addEventListener("DOMContentLoaded", function () {
					...
					if (!dropzoneElement || !submitButton) {
						console.error("Dropzone form or upload button not found");
						return;
					}

					const myDropzone = new Dropzone("#my-dropzone", {
						url: "controllers/image_processor.php?id=<?php echo (int)$recordnumber; ?>&frm=<?php echo (int)$formnumber; ?>",
						paramName: "images",
						maxFiles: <?php echo (int)$maximg > 0 ? (int)$maximg : 20; ?>,
						parallelUploads: <?php echo (int)$maximg > 0 ? (int)$maximg : 20; ?>,
						uploadMultiple: true,
						...
						error: function (file, response) {
							...
							if (responseBox) {
								responseBox.innerHTML = "Upload failed. Check console/network response.";
							}
						}
					});
	...
	<script>
		document.addEventListener("DOMContentLoaded", function () {
			const tabStateKey = 'recordEditv4-tab-<?php echo (int) $formnumber; ?>-<?php echo (int) $recordnumber; ?>';
			const activateTab = function (tab) {
				const target = document.querySelector(tab.getAttribute('href'));
				if (!target) return false;

				document.querySelectorAll('#pills-tab .nav-link').forEach(link => {
			...
			// A link hash deliberately takes priority; otherwise restore the last
			// tab used for this exact form/record during the current browser session.
			const requestedTab = window.location.hash || sessionStorage.getItem(tabStateKey);
			if (requestedTab) {
				const savedTab = document.querySelector('#pills-tab a[href="' + requestedTab + '"]');
			...
			document.addEventListener("click", function (event) {
				const tab = event.target.closest('#pills-tab [data-bs-toggle="pill"]');
				if (!tab) return;

				event.preventDefault();
				if (activateTab(tab)) {
					sessionStorage.setItem(tabStateKey, tab.getAttribute('href'));
				}
			});
		});
	</script>
