...
function fetchDynamicMenuItems($conn, $item, $baseURL) {
    ...
    // Litters are presented together on their listing page rather than having
    // individual detail routes. The dropdown contains only opted-in litters.
    if ($tableName === 'litters') {
        $pageSlug = getPageSlug($conn, $item['page']);
        $listingUrl = rtrim($baseURL, '/') . '/' . $pageSlug;

        $result = mysqli_query($conn, "SELECT id, title FROM litters WHERE showonweb = 'Yes' AND archived = 0 AND showonmenu = 'Yes' ORDER BY dob DESC, id DESC");
        if ($result) {
...
function renderMenu($conn, $baseURL, $placement = 'below') {
    ...
    // Presentation-only mobile break: preferences remain a single site-name value.
    $siteNameParts = preg_split('/\s+/', trim($GLOBALS['prefs']['prefSiteName'] ?? 'Site'), 2);
    $siteNameFirst = $siteNameParts[0] ?: 'Site';
    $siteNameRest = $siteNameParts[1] ?? '';
    ?>
      ...
      <nav class="navbar navbar-expand-lg main-navbar" data-bs-auto-close="outside">
        <div class="container">
          ...
          <a class="navbar-brand text-white fw-semibold d-lg-none" href="<?= $baseURL ?>/">
            ...
            <span class="mobile-brand-name"><span class="mobile-brand-first"><?= htmlspecialchars($siteNameFirst, ENT_QUOTES, 'UTF-8') ?></span><?php if ($siteNameRest !== ''): ?> <span class="mobile-brand-rest"><?= htmlspecialchars($siteNameRest, ENT_QUOTES, 'UTF-8') ?></span><?php endif; ?></span>
          </a>
