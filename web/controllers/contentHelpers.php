...
/** Return the CMS-configured route for the product detail page. */
function getProductDetailRoute(mysqli $conn): string
{
    static $route = null;
    if ($route !== null) {
        return $route;
    ...
    $result = mysqli_query($conn, "SELECT slug FROM pages WHERE id = 6 AND archived = 0 LIMIT 1");
    $row = $result ? mysqli_fetch_assoc($result) : null;
    $route = !empty($row['slug']) ? trim($row['slug'], '/') : '';
    return $route;
}
 ...
 * @param string $folder   Subfolder inside /filestore/images/  (e.g. "content" or "dogs")
...
function renderGallery($conn, $formid, $recordid, $baseURL, $sort = "id ASC", $folder = "content", $productName = "", bool $showFallback = true, string $zoomOptions = 'zoomPosition: inner', string $thumbnailClass = 'col-6 col-md-3 col-lg-3', string $thumbnailSize = 'md') {
    // --- Get gallery records ---
    $selectgallery = "
        SELECT * FROM `gallery`
    ...
    if ($numgallery < 1) {
        ...
        echo "<div class='text-center'>
                <a class='MagicZoom' id='zoom-$recordid' title='No image available' data-options='{$zoomOptions}'
    ...
    // --- Output main image ---
    echo "<a class='MagicZoom' id='zoom-$recordid' title='{$mainCaption}' data-options='{$zoomOptions}' href='{$baseURL}{$mainImgLg}'>
    ...
    if ($numgallery > 1) {
        ...
        while ($rowgallery = mysqli_fetch_assoc($querygallery)) {
            ...
            $imgThumb = getValidImagePath($image, $folder, $thumbnailSize);
            ...
            echo "<div class='{$thumbnailClass}'>";
            echo "<a data-zoom-id='zoom-$recordid'
                 href='{$baseURL}{$imgLg}'
                 data-image='{$baseURL}{$imgMd}'
                 title='{$thumbCaption}'>";
            echo "<img src='{$baseURL}{$imgThumb}' class='img-fluid w-100' alt='{$thumbAlt}'>";
            echo "</a>";
            echo "</div>";
        }
        echo "</div>";
...
function getHorse($conn, $baseURL, $id, $level = 1, $maxGen = 4) {
    ...
    $route = getProductDetailRoute($conn);
    $link = $route !== '' ? $baseURL . '/' . $route . '/' . $row['id'] . '/' . $slug : '';
...
/**
 * Return the first horse image (sm size) or a folder-specific no-image fallback.
 * Uses getValidImagePath() to check existence and select best available image.
 */
/**
 * Return horse image (sm size) or folder-specific no-image fallback.
 * Optional $maxHeight parameter for specific generations (e.g. generation 4).
 * Set $preserveFullImage for a generation that must not use cropped cover styling.
 */

function getHorseImage($conn, $baseURL, $recordid, $maxHeight = null, $preserveFullImage = false) {
    $folder = 'images/dogs';
    $imgFile = '';
    $imageSize = 'sm';
    ...
    if (!empty($recordid) && $recordid > 0) {
        ...
        if ($res && mysqli_num_rows($res) > 0) {
            $row = mysqli_fetch_assoc($res);
            $folder = !empty($row['folder_name'])
                ? trim($row['folder_name'], '/')
                : $folder;
            $imgFile = $row['image'];
        }
    }
    ...
    if (empty($imgFile)) {
        ...
        // Use the same normal pedigree size for the holding image so its
        // "Image coming soon" message remains legible.
        $imageSize = 'sm';
    }

    // Build image URL
    $imgSrc = "{$baseURL}/filestore/{$folder}/{$imageSize}/{$imgFile}";
    ...
    return "<div class='pedigree-thumb mt-2'>
                <img src='{$imgSrc}' alt='' 
                     ...
                     "object-fit:{$objectFit}; object-position:center;'>
            </div>";
}
