...
?>
    <style>
        /* Use a landscape frame while retaining every part of each image. */
        .hover-img {
        overflow: hidden;
        ...
        .horse-image {
        object-fit: contain;      /* show the complete image without cropping */
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $link        = $baseURL . "/" . $productRoute . "/" . $productId . "/" . $productSlug;
    ...
    echo "<div class='card h-100 border-0 shadow-sm'>";
        echo "<a href='{$link}' class='text-decoration-none text-dark'>";
            echo "<div class='ratio ratio-16x9 position-relative hover-img bg-light d-flex align-items-center justify-content-center'>";
                ...
                if (count($images) >= 1) {
                    ...
                    $img1Path = getValidImagePath($img1, 'dogs', 'md');
                    ...
                    if (isset($images[1]) && $images[1] !== $images[0]) {
                        ...
                        $img2Path = getValidImagePath($img2, 'dogs', 'md');
                    ...
                    // fallback no image
                    $fallbackPath = getValidImagePath('', 'dogs', 'md');
